...
// end timegroup check
        ...
        if ($trunkallowed) //to save time, if the call is already denied ignore the following
        {
                ...
                switch ($destrunk_tech)
                {
                        ...
                        case 'pjsip':
                                if ($count_inbound) {
                                        $channel_filter="(dstchannel LIKE 'PJSIP/".$destrunk_channelid."%' OR channel LIKE 'PJSIP/".$destrunk_channelid."%')";
                                } else {
                                        $channel_filter="dstchannel LIKE 'PJSIP/".$destrunk_channelid."%'";
                                }
                        break;
                ...
                if (($maxtime>0) and ($trunkallowed))
                        {
                        $sql='SELECT ROUND(SUM(billsec/60)+0.5)  FROM `cdr` WHERE '.$disposition.' AND '.$channel_filter.' '.$sqldate.$sqlpattern;
                        $query= $db2->sql($sql,'NUM');
